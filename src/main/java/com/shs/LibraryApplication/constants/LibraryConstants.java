package com.shs.LibraryApplication.constants;

public class LibraryConstants {

    // URLs
    public static final String HOME_URL = "/";
    public static final String ADMIN_DASHBOARD_URL = "/admin";
    public static final String LOGIN_URL = "/user/login";
    public static final String AUTHOR_LIST_URL = "/author/list";
    public static final String BOOK_LIST_URL = "/book/list";

    // Freemarker Templates
    public static final String BOOKS_LIST_TEMPLATE = "books-list";
    public static final String LOGIN_TEMPLATE = "login";
    public static final String SIGNUP_TEMPLATE = "signup";
    public static final String HOME_PAGE_TEMPLATE = "home";
    public static final String ADMIN_DASHBOARD_TEMPLATE = "admin-dashboard";
    public static final String AUTHORS_LIST_TEMPLATE = "authors-list";
    public static final String USERS_LIST_TEMPLATE = "users-list";
    public static final String ERR_404_TEMPLATE = "error-404";
    public static final String ERR_500_TEMPLATE = "error-500";

}
